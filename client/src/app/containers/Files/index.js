...
export class Files extends React.Component {
    ...
    handleClick(item, claims) {
        let fileDetails = [
            ...
            { key: 'File Date', value: moment(item.FileDate).format('DD/MM/YYYY') + moment(item.FileDate).format(' h:m A') },
        ...
        let claimsDetails = [
            ...
            { key: 'Accident Date', value: moment(claims.ClaimExtNmbr).format('DD/MM/YYYY') },
    ...
    renderList() {
        ...
        Object.keys(data).map((keys) => {
            row.push(
                <div className="row">
                    <div className="col-4 col-style"><a href={"#" + keys} style={{ color: "#6AA2B8" }} data-toggle="collapse" aria-expanded="false">{data[keys].value.FileName}</a></div>
                    <div className="col-2 col-style">{moment(data[keys].value.FileDate).format('DD/MM/YYYY')}<br />{moment(data[keys].value.FileDate).format('h:m a')}</div>
                    <div className={"col-2 col-style " + (data[keys].value.FExtraField2 == 'Errors' || data[keys].value.FExtraField2 == 'File Error' ? 'red ' : (data[keys].value.FExtraField2 == 'Verified' ? 'green ' : ''))}>{data[keys].value.FExtraField2}</div>
                    <div className="col-2 col-style">{data[keys].value.FSubmitter_N103}</div>
                    <div className="col-2 col-style">{data[keys].value.FReceiver_N103}</div>
                </div>
            )
